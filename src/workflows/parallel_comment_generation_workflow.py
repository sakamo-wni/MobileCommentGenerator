...
def create_parallel_comment_generation_workflow() -> StateGraph:
    """並列処理対応の天気コメント生成ワークフローを構築"""
...
def run_parallel_comment_generation(
    location_name: str,
    target_datetime: Optional[datetime] = None,
    llm_provider: str = "openai",
    exclude_previous: bool = False,
    use_unified_mode: bool = False,
    **kwargs,
) -> Dict[str, Any]:
    ...
    Args:
        ...
        use_unified_mode: 統合モードを使用するか
        **kwargs: その他のオプション
        
    Returns:
        生成結果を含む辞書
    """
    logger.info(f"並列ワークフロー実行: unified_mode={use_unified_mode}")
    
    workflow = create_parallel_comment_generation_workflow()
